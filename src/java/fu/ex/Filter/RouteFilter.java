...
public class RouteFilter implements Filter {
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
        ...
        HttpSession session = httpResp.getSession();
        MemberDAO mdao = new MemberDAO();
        Member m = (Member) session.getAttribute("member") != null ? (Member) session.getAttribute("member") : null;

        // if status == 0, redirect to 404 page
        try{
            if (m != null) {
                ...
                if (!nm.isStatus()) {
                    request.getRequestDispatcher("/banned.jsp").forward(request, response);
                    return;
                }
            }
        } catch (Exception e) {
